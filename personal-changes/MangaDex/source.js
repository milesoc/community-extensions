...
var _Sources = (() => {
  ...
  var parseMangaList = async (object, source, thumbnailSelector) => {
    ...
    for (const manga of object) {
      ...
      const rawTitle = source.decodeHTMLEntity(mangaDetails.title.en ?? mangaDetails.altTitles.map((x) => Object.values(x).find((v) => v !== void 0)).find((t) => t !== void 0));
  ...
  var addFileNamesToManga = async (manga, covers, source, thumbnailSelector) => {
    const thumbnailSelectorState = await thumbnailSelector(source.stateManager);
  ...
  var getCountryFlag = (language) => {
    ...
    const codePoints = Array.from(
      countryCode,
      (char) => 127462 - 65 + char.charCodeAt(0)
    );
  ...
  var MangaDex = class {
    ...
    async getHomePageSections(sectionCallback) {
      ...
      const sections = [
        ...
        {
          request: App.createRequest({
            url: new URLBuilder(this.MANGADEX_API).addPathComponent("chapter").addQueryParameter("limit", 20).addQueryParameter("translatedLanguage", languages).addQueryParameter("order", { readableAt: "desc" }).addQueryParameter("contentRating", ratings).addQueryParameter("includes", ["manga"]).buildUrl(),
            method: "GET"
          }),
          section: App.createHomeSection({
            id: "latest_updates",
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      switch (homepageSectionId) {
        ...
        case "latest_updates": {
          url = new URLBuilder(this.MANGADEX_API).addPathComponent("chapter").addQueryParameter("limit", 20).addQueryParameter("translatedLanguage", languages).addQueryParameter("order", { readableAt: "desc" }).addQueryParameter("contentRating", ratings).addQueryParameter("offset", offset).addQueryParameter("contentRating", ratings).addQueryParameter("includes", ["manga"]).buildUrl();
          break;
        }
        case "recently_Added": {
          url = new URLBuilder(this.MANGADEX_API).addPathComponent("manga").addQueryParameter("limit", 100).addQueryParameter("hasAvailableChapters", true).addQueryParameter("availableTranslatedLanguage", languages).addQueryParameter("order", { createdAt: "desc" }).addQueryParameter("offset", offset).addQueryParameter("contentRating", ratings).addQueryParameter("includes", ["cover_art"]).buildUrl();
    ...
    async appendCoverArt(sourceMangaAsync, thumbnailSelector, offset = 0) {
      return sourceMangaAsync.then((sourceManga) => {
        const request = App.createRequest({
          url: new URLBuilder(this.MANGADEX_API).addPathComponent("cover").addQueryParameter("manga", sourceManga.map((manga) => manga.mangaId)).addQueryParameter("includes", ["manga"]).addQueryParameter("limit", 100).addQueryParameter("offset", offset).addQueryParameter("order", { volume: "asc" }).buildUrl(),
          method: "GET"
        });
        ...
        return asyncResponse.then(async (response) => {
          ...
          const offset2 = Number(json.offset);
          if (total > limit + offset2) {
            return addFileNamesToManga(sourceManga, json.data, this, thumbnailSelector).then((manga) => {
              return this.appendCoverArt(Promise.resolve(manga), thumbnailSelector, offset2 + limit);
            });
          }
